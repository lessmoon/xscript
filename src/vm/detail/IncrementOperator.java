 ...
 * DecrementOperator
 */
public class IncrementOperator extends OutputBinaryOperator {
    final Type type;

    public IncrementOperator(ReadOnlyPointer input, WriteOnlyPointer output, Type type) {
        super(input, output);
        this.type = type;
    }
    ...
    protected Value calculate(VirtualMachine vm, Value input) {
        ...
        case BIGINT:
            return new Value((((BigNum) op).value).add(BigInteger.ONE));
        case CHAR:
