...
public class ExtensionStructHelper {
    ...
    /**
     * Generated function proxy invocation without return For with-return version
     * use
     * {@linkplain #functionBodyWithReturn(StructValue, int, boolean, StackVar[], Type, Method, boolean, symbols.Struct)}
     *
     ...
     */
    private static void functionBodyNoReturn(final StructValue arg0, final int argLength,
            final boolean needPassThisReference, final StackVar[] vars, final Method method) {
    ...
    /**
     * Generate function invocation proxy body of java method(with return) For
     * no-return version use
     * {@linkplain #functionBodyNoReturn(StructValue, int, boolean, StackVar[], Method)}
     *
     * @param arg0                  this reference
     * @param argLength             the args number of methods in java
     * @param needPassThisReference if we should pass this to the method
     * @param vars                  the stack vars(vm stack frame)
     * @param returnType            the return type(may have some dynamic cast work
     *                              to do)
     * @param method                the bound method
     * @param retFlag               if we should dynamic cast the return type
     * @param s                     the Struct type
     */
    private static void functionBodyWithReturn(final StructValue arg0, final int argLength,
            final boolean needPassThisReference, final StackVar[] vars, final Type returnType, final Method method,
            final boolean retFlag, final symbols.Struct s) {
     ...
     * @param needPassThis if this method annotated to be passed
     *                     this{@linkplain PassThisReference}
     * @param typeTable    the type table
     * @param dic          the token dictionary
     * @param clazz        the clazz
     * @return the param generated by this procedure
     */
    private static <T> List<Param> checkAndBuildParams(final StackVar[] vars, symbols.Struct s, Method m, String[] args,
            final boolean needPassThis, TypeTable typeTable, Dictionary dic, Class<T> clazz) {
        ...
        for (int i = 0; i < args.length; i++, j++) {
            ...
            assert (t != Type.Void);
            assert (t != null) : ("declared arg[" + (i + 1) + "] of `" + s.getName() + "." + m.getName() + "()' type["
                    + typeStr + "] is not found");
            assert (Value.class.isAssignableFrom(ps[j].getType())) : ("implementation of declared arg[" + (i + 1)
                    + "] of `" + s.getName() + "." + m.getName() + "()':can't bind type[" + t + "] to ["
                    + ps[j].getType().getName() + "] ");

            if (ArrayValue.class.isAssignableFrom(ps[j].getType())) {
                // check if it is an array arg declared
                assert (t instanceof Array) : ("implementation of declared arg[" + (i + 1) + "] of `" + s.getName()
                        + "." + m.getName() + "()':can't bind type[" + t + "] to [" + ps[j].getType().getName() + "] ");
            } else if (Struct.class.isAssignableFrom(ps[j].getType())) {
                // check if it is an array arg declared
                assert (t instanceof symbols.Struct) : ("implementation of declared arg[" + (i + 1) + "] of `"
                        + s.getName() + "." + m.getName() + "()':can't bind type[" + t + "] to ["
                        + ps[j].getType().getName() + "] ");
            }
    ...
    /**
     * Get type by {@code typeStr} The type is either Basic
     * type(int,real,string,bool,void,bigreal and bigint) or Struct type. todo:Array
     * type is not tested yet. Some special case: 1.Type strings starting with "#"
     * means find struct bound by the class named by the substring after # 2.Type
     * string starting with "#." means relatively searching the class(based on
     * package of the class) 3.Type string equaling "$" means the struct bound to
     * the class e.g: "int" -> Type.Int "Runnable" -> Struct Runnable In context of
     * class {@linkplain extension.system.SimpleFileInputStream}, "$" -> The struct
     * bound to the class {@linkplain extension.system.SimpleFileInputStream}
     * "#extension.system.SimpleFile" -> The struct bound to the class
     * SimpleFile,note that full name of class
     * {@linkplain extension.system.SimpleFile} "#.SimpleFile" -> search in package
     * extension.system(where the
     * {@linkplain extension.system.SimpleFileInputStream} is located)
     *
     ...
     * @return the type
     */
    private static <T> Type getType(String typeStr, symbols.Struct s, Dictionary dic, TypeTable typeTable,
            Class<T> clazz) {
        Type t;
        /*
         * Syntax: Type -> BasicType | BasicType Array BasicType ->
         * ID|#.ID.PATH|#PATH|$PATH Array -> [] | [] Array PATH -> ID | ID.PATH
         */
     ...
     */
    private static <T> MemberFunction getMemberFunction(final Method m, final Class<T> clazz, Dictionary dic,
            TypeTable typeTable, symbols.Struct s) {
        ...
        final Parameter[] ps = m.getParameters();
        assert !needPassThisReference || ps.length > 0
                && (ps[0].getType().equals(StructValue.class) || ps[0].getType().equals(Value.class)) : "Annotation `"
                        + PassThisReference.class.getSimpleName() + "' requires first parameter to be `"
                        + Value.class.getSimpleName() + "' or `" + StructValue.class.getSimpleName() + "' in " + m;

        assert argLength == m.getParameterCount() : "member function `" + m + "' for '" + s.toString()
                + "''s parameter length not matched";
        assert !Modifier.isStatic(m.getModifiers()) : "try to bind with static function" + m + " to " + s;

        final StackVar[] vars = new StackVar[paraLength];
        final List<Param> param = checkAndBuildParams(vars, s, m, func.param(), needPassThisReference, typeTable, dic,
                clazz);
        ...
        assert (returnType == Type.Void || Value.class.isAssignableFrom(m.getReturnType())) : // check if the return
                                                                                              // type
        "return type `" + m.getReturnType() + "' of " + m.getName() + " is not assignable to "
                + Value.class.getSimpleName() + " and declared return is not void";

        // check if it is an array
        if (m.getReturnType().isInstance(ArrayValue.class)) {
            assert (returnType instanceof Array) : "implementation of return type of " + m.getName()
                    + ":can't bind type[" + returnType + "] to [" + m.getReturnType().getName() + "] ";
        }

        if (m.getReturnType().isInstance(StructValue.class)) {
            // check if it is an Struct
            assert (returnType instanceof symbols.Struct) : "implementation of return type of " + m.getName()
                    + ":can't bind type[" + returnType + "] to [" + m.getReturnType().getName() + "] ";
        }

        Stmt body = func.purevirtual() ? null : returnType == Type.Void ? new Stmt() {
            ...
            @Override
            public void run() {
                functionBodyWithReturn((StructValue) arg0.getValue(), argLength, needPassThisReference, vars,
                        returnType, m, retFlag, s);
            }
        };
            ...
            if (methodAnnotation != null) {
                ...
                if (methodAnnotation.virtual() || methodAnnotation.purevirtual()) {
                    ...
                    if (methodAnnotation._default()) {
                        assert s.getDefaultFunctionName() == null
                                || s.getDefaultFunctionName() == f.getName() : "default function is already set:"
                                        + s.getDefaultFunctionName();
     ...
     * @param           <T> the Type
     * @return the struct
     * @see #buildStructFromClass(Class, Dictionary, TypeTable, symbols.Struct,
     *      boolean)
     */
    public static <T> symbols.Struct buildStructFromClass(Class<T> c, Dictionary dic, TypeTable typeTable, Token name,
            boolean needDefaultInit) {
     ...
     * @param typeTable the type table
     * @param s         the struct
     * @param           <T> the Class type(to bind,it must have a default
     *                  constructor)
     * @return the initial function if ok or null
     */
    private static <T> InitialFunction getInitialFunction(final Method m, final Class<T> clazz, Dictionary dic,
            TypeTable typeTable, symbols.Struct s) {
        ...
        PassThisReference tmp = m.getAnnotation(PassThisReference.class);
        final boolean needPassThisReference = tmp != null && tmp.value();
        int j = needPassThisReference ? 1 : 0;// goes for first parameter of implementation methods args bind for s.func
                                              // arg0(always 0 or 1)
        final int paraLength = init.param().length;
        final int argLength = paraLength + j;
        final Parameter[] ps = m.getParameters();
        assert !needPassThisReference || ps.length > 0
                && (ps[0].getType() == StructValue.class || ps[0].getType() == Value.class) : "Annotation `"
                        + PassThisReference.class.getSimpleName() + "' requires first parameter to be `"
                        + Value.class.getSimpleName() + "' or `" + StructValue.class.getSimpleName() + "' in " + m;
        assert argLength == m.getParameterCount() : "setBody function for " + s.toString()
                + "'s parameter length not matched";
        assert !Modifier.isStatic(m.getModifiers());

        final StackVar[] vars = new StackVar[paraLength];
        final List<Param> param = checkAndBuildParams(vars, s, m, init.param(), needPassThisReference, typeTable, dic,
                clazz);
        ...
        return new InitialFunction(Word.This, param, s, new Stmt() {
            ...
            public void run() {
                StructValue s1 = (StructValue) arg0.getValue();
                try {
                    s1.setExtension(clazz.getDeclaredConstructor().newInstance());
    ...
    /**
     * build a default initial function,it just construct an instance of clazz and
     * set the s.extension
     *
     * @param clazz     the class built from
     * @param dic       the dictionary
     ...
     */
    private static <T> InitialFunction getDefaultInitialFunction(final Class<T> clazz, Dictionary dic,
            TypeTable typeTable, symbols.Struct s) {
        final StackVar arg0 = new StackVar(Word.This, s, 0, 0);
        ...
        Stmt body = new Stmt() {
            ...
            public void run() {
                try {
                    ((StructValue) arg0.getValue()).setExtension(clazz.getDeclaredConstructor().newInstance());
                } catch (NoSuchMethodException | InvocationTargetException | InstantiationException | IllegalAccessException e) {
                    throw new RuntimeException(e);
                }
            }
