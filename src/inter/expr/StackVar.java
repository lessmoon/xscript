...
public class StackVar extends Var {
    ...
    public boolean equals(Object o) {
        return this == o || o instanceof StackVar && ((StackVar) o).offset == offset
                && ((StackVar) o).stackLevel == stackLevel && ((StackVar) o).type.isCongruentWith(this.type);
    }

    @Override
    public int hashCode() {
        // NOTE:Type should join the hash code computing,because it is not necessary to
        // be same hashCode between two same types
    ...
    public static StackVar stackVar(Token id, Type type, int stackLevel, int stackOffset, int toplevel) {
        return stackLevel == 0 ? new AbsoluteVar(id, type, 0, stackOffset)
                : new StackVar(id, type, toplevel - stackLevel, stackOffset);
    }
}